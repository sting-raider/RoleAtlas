...
import { splitList } from "./listText";
  ...
  return (
    <div className="daily-page home-workspace" aria-busy={restoring}>
      ...
      <section className="daily-metric-grid" aria-label="Daily summary">
        <button type="button" onClick={() => onNavigate("discover")}><span className="metric-accent"><Sparkles size={17} /></span><strong>{summary.strongMatches}</strong><span>strong matches</span><small>{summary.newSinceLastVisit} new since last visit</small></button>
        <button type="button" onClick={() => onNavigate("searches")}><span className="metric-accent"><Radar size={17} /></span><strong>{summary.activeSearches}</strong><span>active searches</span><small>{summary.expansionAdded} listings observed in expansion</small></button>
        <button type="button" onClick={() => onNavigate("applications")}><span className="metric-accent"><Clock3 size={17} /></span><strong>{summary.applicationsNeedingAction}</strong><span>applications need action</span><small>Follow-ups and next steps</small></button>
        <button type="button" onClick={() => onNavigate("sources")}><span className="metric-accent"><Globe2 size={17} /></span><strong>{summary.coverageIssues}</strong><span>coverage issues</span><small>{summary.savedJobsClosed} saved jobs closed</small></button>
      </section>
...
type ContactDraft = { name: string; detail: string };

function contactText(contact: ContactDraft | null): string {
  return contact ? `${contact.name} — ${contact.detail}` : "";
}

function parseContact(raw: string): ContactDraft | null {
  if (!raw.trim()) return null;
  const [name, ...detail] = raw.split("—");
  return { name: name.trim(), detail: detail.join("—").trim() };
}

function sameContact(a: ContactDraft | null, b: ContactDraft | null): boolean {
  if (!a || !b) return !a && !b;
  return a.name === b.name && a.detail === b.detail;
}

// Same raw-buffer contract as ListInput: keystrokes are never rewritten
// underneath the user ("J" must not become "J — " mid-word); parsing happens
// only to commit upward, and props win solely on genuine external changes.
function ContactInput({ contact, onChange }: { contact: ContactDraft | null; onChange: (contact: ContactDraft | null) => void }) {
  const [raw, setRaw] = useState(() => contactText(contact));
  const [seen, setSeen] = useState<ContactDraft | null>(contact);
  if (contact !== seen && !sameContact(contact, seen)) {
    setSeen(contact);
    if (!sameContact(parseContact(raw), contact)) setRaw(contactText(contact));
  }
  const handleInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    const next = event.target.value;
    setRaw(next);
    const parsed = parseContact(next);
    setSeen(parsed);
    onChange(parsed);
  };
  return <input type="text" value={raw} onChange={handleInput} placeholder="Name — email, profile, or role" aria-label="Contact" />;
}
...
export function SearchesWorkspace({ strategies, sessions, onSave, onDuplicate, onStatus, onRerun }: { strategies: StrategyRecord[]; sessions: SessionSummary[]; onSave: (plan: SearchPlan, strategyId?: string) => void; onDuplicate: (strategyId: string) => void; onStatus: (strategyId: string, status: StrategyStatus) => void; onRerun: (strategy: StrategyRecord) => Promise<void> }) {
  ...
  const [targetCountryInput, setTargetCountryInput] = useState("");
  ...
  const countryCodes = (value: string) => [...new Set(splitList(value).map((item) => resolveLiteCountry(item)?.code ?? null).filter((code): code is string => Boolean(code)))];
  return (
    <div className="daily-page searches-workspace">
      ...
      {editing && draftPlan && <div className="workspace-dialog-backdrop"><section className="workspace-dialog strategy-editor" role="dialog" aria-modal="true" aria-labelledby="strategy-editor-title"><header><div><span className="eyebrow">Human-approved plan</span><h2 id="strategy-editor-title">Edit search strategy</h2></div><button type="button" className="icon-button" aria-label="Close strategy editor" onClick={() => setEditing(null)}><X size={17} /></button></header><div className="workspace-form"><label><span>Name</span><input value={draftPlan.strategyName ?? ""} onChange={(event) => setDraftPlan({ ...draftPlan, strategyName: event.target.value })} /></label><label><span>Primary and adjacent role queries</span><ListInput multiline rows={3} ariaLabel="Primary and adjacent role queries" values={draftPlan.roleQueries} onChange={(nextValues) => setDraftPlan({ ...draftPlan, roleQueries: nextValues })} /></label><div className="field-grid two"><label><span>Title synonyms</span><ListInput ariaLabel="Title synonyms" values={draftPlan.titleSynonyms ?? []} onChange={(nextValues) => setDraftPlan({ ...draftPlan, titleSynonyms: nextValues })} /></label><label><span>Excluded terms</span><ListInput ariaLabel="Excluded terms" values={draftPlan.excludedTerms ?? []} onChange={(nextValues) => setDraftPlan({ ...draftPlan, excludedTerms: nextValues })} /></label><label><span>Target countries</span><input value={targetCountryInput} onChange={(event) => setTargetCountryInput(event.target.value)} placeholder="United Kingdom, Germany" /></label><label><span>Additional source regions or cities</span><ListInput ariaLabel="Additional source regions or cities" values={draftPlan.locations} onChange={(nextValues) => setDraftPlan({ ...draftPlan, locations: nextValues })} placeholder="EMEA, Berlin" /></label><label><span>Freshness days</span><input type="number" min="1" max="365" value={draftPlan.freshnessDays ?? 30} onChange={(event) => setDraftPlan({ ...draftPlan, freshnessDays: Number(event.target.value) || 30 })} /></label></div><div className="inference-note"><Sparkles size={16} /><p>Regenerate uses confirmed profile fields deterministically. Model suggestions, when requested in Settings, are previews until you approve them.</p></div></div><footer><button type="button" className="secondary-button" onClick={() => setEditing(null)}>Cancel</button><button type="button" className="primary-button" disabled={!draftPlan.roleQueries.length} onClick={() => { const preferredCountryCodes = countryCodes(targetCountryInput); onSave({ ...draftPlan, mobility: { ...draftPlan.mobility, preferredCountryCodes }, targetCountryCodes: preferredCountryCodes }, editing); setEditing(null); }}><Save size={15} /> Save revision</button></footer></section></div>}
...
export function SourcesWorkspace() {
  ...
  return <div className="daily-page sources-workspace"><div className="daily-hero"><div><span className="eyebrow">Honest configured coverage</span><h1>Sources</h1><p>See what RoleAtlas is actually configured to check. This is not a claim of whole-market coverage.</p></div></div><div className="source-explainer"><ShieldCheck size={19} /><p><strong>Source relevance can select a board for scanning, but cannot confirm your eligibility for a listing.</strong> Listing-level geography and authorization evidence decide that. Source failures may reduce results.</p></div>{loading ? <div className="daily-loading" role="status">Checking configured sources…</div> : <div className="source-table" role="table" aria-label="Configured job sources"><div role="row" className="source-table-head"><span role="columnheader">Source</span><span role="columnheader">Adapter</span><span role="columnheader">Hiring metadata</span><span role="columnheader">Last scan</span><span role="columnheader">Health</span></div>{registry.map((source) => { const state = health[source.id]; const healthLabel = state?.last_status ?? source.health ?? "unscanned"; return <div role="row" key={source.id}><span role="cell"><strong>{registryCompanyLabel(source.company)}</strong><small>{source.id}</small></span><span role="cell">{source.adapter}</span><span role="cell">{[...source.hiringCountryCodes, ...source.hiringRegionCodes].join(" · ") || "Listing evidence"}</span><span role="cell">{relativeTime(state?.last_success_at)}</span><span role="cell"><em className={`source-state ${healthLabel === "success" ? "healthy" : healthLabel === "failed" ? "failed" : "experimental"}`}>{healthLabel}</em>{state?.observed_jobs ? <small>{state.observed_jobs} latest listings</small> : null}{state?.error ? <small>{state.error}</small> : null}</span></div>; })}</div>}<section className="daily-card source-submit"><div><span className="eyebrow">Validation request</span><h2>Submit a careers URL</h2><p>The URL is validated and queued only through your local stack. Model-generated URLs never enter the trusted registry automatically.</p></div><div><label><span>Employer careers URL</span><input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://company.example/careers" /></label><button type="button" className="primary-button" disabled={!/^https?:\/\//i.test(url)} onClick={() => void submit()}>Submit for validation</button>{message && <p role="status">{message}</p>}</div></section></div>;
}

export function SettingsWorkspace({ provider, onProvider, aiActivities, status, onRefreshStatus, onStartOnboarding, onResetLearned, currentUser }: { provider: ProviderConfig; onProvider: () => void; aiActivities: Array<{ id: string; action: string; provider: string; model: string; completedAt: string; outcome: string; dataSent: string[] }>; status: ServiceStatus; onRefreshStatus: () => void; onStartOnboarding: () => void; onResetLearned: () => void; currentUser: { id: string; name: string; email: string } }) {
