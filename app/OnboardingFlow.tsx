    ...
    <div className="onboarding-backdrop" role="presentation">
      <section ref={dialogRef} tabIndex={-1} className="onboarding-shell" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
        <aside className="onboarding-progress" aria-label="Onboarding progress">
          ...
          <nav className="onboarding-step-list" aria-label="Setup steps">
            {ONBOARDING_STEPS.map((step, position) => (
              <div key={step} className={position === index ? "current" : position < index || draft.completedSteps.includes(step) ? "complete" : ""}>
          ...
          <div className="onboarding-content">
            ...
            {draft.currentStep === "review-facts" && profile && (
              <div className="onboarding-form">
                ...
                <label><span>Skills and evidence <em>{profile.skills.some((skill) => !skill.confirmed) ? "Inferred fields" : "Confirmed"}</em></span><ListInput multiline rows={3} ariaLabel="Skills and evidence" values={profile.skills.map((skill) => skill.value)} onChange={(nextValues) => { const next = nextValues.map((value) => profile.skills.find((skill) => skill.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)); updateProfile({ ...profile, skills: next, facts: { ...(profile.facts!), skills: next } }); }} /></label>
                <div className="field-grid two">
                  <label><span>Education</span><ListInput ariaLabel="Education" values={(profile.facts?.education ?? []).map((field) => field.value)} onChange={(nextValues) => updateProfile({ ...profile, facts: { ...(profile.facts!), education: nextValues.map((value) => profile.facts?.education?.find((field) => field.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)) } })} placeholder="Degree, programme, or training" /></label>
                  <label><span>Certifications or licenses</span><ListInput ariaLabel="Certifications or licenses" values={(profile.facts?.certifications ?? []).map((field) => field.value)} onChange={(nextValues) => updateProfile({ ...profile, facts: { ...(profile.facts!), certifications: nextValues.map((value) => profile.facts?.certifications?.find((field) => field.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)) } })} placeholder="Only credentials you hold" /></label>
                </div>
            ...
            {draft.currentStep === "career-goals" && profile && (
              <div className="onboarding-form">
                <label><span>Primary role families <em>Required</em></span><ListInput multiline ariaLabel="Primary role families" values={(profile.goals?.primaryRoleFamilies ?? profile.targetRoles).map((field) => field.value)} onChange={(primary) => updateProfile({ ...profile, targetRoles: primary.map((value) => profile.targetRoles.find((field) => field.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)), goals: { ...(profile.goals!), primaryRoleFamilies: primary.map((value) => profile.goals?.primaryRoleFamilies?.find((field) => field.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)) } })} placeholder="Policy research, product design, laboratory operations…" /></label>
                <label><span>Adjacent role families <em>Optional</em></span><ListInput multiline ariaLabel="Adjacent role families" values={(profile.goals?.adjacentRoleFamilies ?? []).map((field) => field.value)} onChange={(nextValues) => updateProfile({ ...profile, goals: { ...(profile.goals!), adjacentRoleFamilies: nextValues.map((value) => profile.goals?.adjacentRoleFamilies?.find((field) => field.value.toLowerCase() === value.toLowerCase()) ?? blankEvidence(value)) } })} placeholder="Related paths you would genuinely consider" /></label>
            ...
            {draft.currentStep === "location-eligibility" && profile && (
              <div className="onboarding-form">
                <datalist id="country-options">{LITE_COUNTRIES.map((item) => <option key={item.code} value={item.name} />)}</datalist>
                <div className="field-grid two">
                  <label><span>Country of residence <em>Not authorization</em></span><input list="country-options" value={liteCountryByCode(profile.mobility.residenceCountryCode)?.name ?? ""} onChange={(event) => { const code = resolveLiteCountry(event.target.value)?.code ?? null; updateProfile({ ...profile, mobility: { ...profile.mobility, residenceCountryCode: code, confirmedFields: [...new Set([...profile.mobility.confirmedFields, "residenceCountryCode"])], inferredFields: profile.mobility.inferredFields.filter((field) => field !== "residenceCountryCode") } }); }} placeholder="Choose or type a country" /></label>
                  <label><span>Target countries</span><ListInput ariaLabel="Target countries" values={profile.mobility.preferredCountryCodes.map((code) => liteCountryByCode(code)?.name ?? code)} onChange={(nextValues) => { const codes = nextValues.map((value) => resolveLiteCountry(value)?.code ?? value); updateProfile({ ...profile, mobility: { ...profile.mobility, preferredCountryCodes: codes, confirmedFields: [...new Set([...profile.mobility.confirmedFields, "preferredCountryCodes"])] } }); }} placeholder="Any country, or list targets" /></label>
                  <label><span>Countries where you already have work authorization <em>Never inferred</em></span><ListInput ariaLabel="Countries where you already have work authorization" values={profile.mobility.workAuthorizedCountryCodes.map((code) => liteCountryByCode(code)?.name ?? code)} onChange={(nextValues) => { const codes = nextValues.map((value) => resolveLiteCountry(value)?.code ?? value); updateProfile({ ...profile, mobility: { ...profile.mobility, workAuthorizedCountryCodes: codes, confirmedFields: [...new Set([...profile.mobility.confirmedFields, "workAuthorizedCountryCodes"])] } }); }} /></label>
                  <label><span>Countries where sponsorship is required <em>Never inferred</em></span><ListInput ariaLabel="Countries where sponsorship is required" values={profile.mobility.requiresSponsorshipCountryCodes.map((code) => liteCountryByCode(code)?.name ?? code)} onChange={(nextValues) => { const codes = nextValues.map((value) => resolveLiteCountry(value)?.code ?? value); updateProfile({ ...profile, mobility: { ...profile.mobility, requiresSponsorshipCountryCodes: codes, confirmedFields: [...new Set([...profile.mobility.confirmedFields, "requiresSponsorshipCountryCodes"])] } }); }} /></label>
                </div>
                ...
                <div className="field-grid two">
                  <label><span>Preferred timezones</span><ListInput ariaLabel="Preferred timezones" values={profile.mobility.preferredTimezones} onChange={(nextValues) => updateProfile({ ...profile, mobility: { ...profile.mobility, preferredTimezones: nextValues, confirmedFields: [...new Set([...profile.mobility.confirmedFields, "preferredTimezones"])] } })} placeholder="Asia/Kolkata, Europe/London…" /></label>
            ...
            {draft.currentStep === "hard-constraints" && profile && (
              <div className="onboarding-form">
                ...
                <label><span>Excluded title terms</span><ListInput ariaLabel="Excluded title terms" values={profile.constraints?.excludedTerms ?? []} onChange={(nextValues) => updateProfile({ ...profile, constraints: { ...(profile.constraints!), excludedTerms: nextValues } })} placeholder="Senior, commission-only…" /></label>
                <label><span>Companies to exclude</span><ListInput ariaLabel="Companies to exclude" values={profile.constraints?.excludedCompanies ?? []} onChange={(nextValues) => updateProfile({ ...profile, constraints: { ...(profile.constraints!), excludedCompanies: nextValues } })} /></label>
            ...
            {draft.currentStep === "strategy-preview" && profile && plan && (
              <div className="onboarding-form strategy-preview-form">
                ...
                <label><span>Search queries</span><ListInput multiline rows={3} ariaLabel="Search queries" values={plan.roleQueries} onChange={(nextValues) => commit({ strategy: { ...plan, roleQueries: nextValues } })} /></label>
                <div className="field-grid two"><label><span>Title synonyms</span><ListInput ariaLabel="Title synonyms" values={plan.titleSynonyms ?? []} onChange={(nextValues) => commit({ strategy: { ...plan, titleSynonyms: nextValues } })} /></label><label><span>Excluded terms</span><ListInput ariaLabel="Excluded terms" values={plan.excludedTerms ?? []} onChange={(nextValues) => commit({ strategy: { ...plan, excludedTerms: nextValues } })} /></label></div>
