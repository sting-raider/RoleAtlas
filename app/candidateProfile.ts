...
import { liteCountryByCode, resolveLiteCountry } from "../shared/geography-lite.ts";
...
type ResumeInput = {
  ...
  /** Server-resolved ISO code from the résumé route (full-corpus match). */
  locationCountryCode?: string | null;
  /** Server-resolved IANA timezone from the résumé route. */
...
export function buildCandidateProfile(resume: ResumeInput): CandidateProfile {
  ...
  // The résumé route resolves the location server-side with the full
  // geography corpus; the client re-resolves only exact country names/codes
  // (plus one comma-token fallback) so the heavy dataset never enters the
  // browser bundle. Server-resolved codes ride along when available.
  const resolvedCountry = resume.location
    ? resolveLiteCountry(resume.location)
      ?? resume.location.split(",").reverse().map((token) => token.trim()).map(resolveLiteCountry).find(Boolean) ?? null
    : null;
  const residenceCountryCode = resume.locationCountryCode ?? resolvedCountry?.code ?? null;
  ...
  const mobility = {
    ...
    workAuthorizedCountryCodes: [],
    requiresSponsorshipCountryCodes: [],
    preferredCountryCodes: residenceCountryCode ? [residenceCountryCode] : [],
    excludedCountryCodes: [],
    preferredCities: resume.location ? [{ raw: resume.location, city: null, subdivisionCode: null, subdivisionName: null, countryCode: residenceCountryCode, regionCodes: [], timezone: resume.locationTimezone ?? null, confidence: 0.5, evidence: [] }] : [],
    willingToRelocate: false,
    relocationCountryCodes: [],
    preferredTimezones: resume.locationTimezone ? [resume.locationTimezone] : [],
    maximumTimezoneDifferenceHours: null,
    inferredFields: residenceCountryCode ? ["residenceCountryCode", "preferredCountryCodes", "preferredCities", ...(resume.locationTimezone ? ["preferredTimezones"] : [])] : [],
    confirmedFields: [],
...
export function searchPlanGeographyLabel(plan: SearchPlan): string {
  const preferredCountries = [...new Set((plan.mobility?.preferredCountryCodes ?? []).map((code) => liteCountryByCode(code)?.name ?? code.trim()).filter(Boolean))];
