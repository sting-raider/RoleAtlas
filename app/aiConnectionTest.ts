...
export async function testProviderConnection(
  request: Request,
  onActivity?: (activity: AiActivity) => Promise<unknown>,
) {
  const startedAt = new Date().toISOString();
  let config: ProviderConfig | undefined;
  let endpoint = "";
  const respond = async (payload: Record<string, unknown>, init?: ResponseInit) => {
    const activity = payload.activity as AiActivity | undefined;
    if (activity && onActivity) await onActivity(activity);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return respond({ error: providerMessage, activity: activityFrom(config, "connection_test", endpoint, startedAt, "failed", ["API credential", "configured model name"], { message: providerMessage }) }, { status: 502 });
    }
    ...
    if (!modelFound) {
      ...
      return respond({ error: message, availableModels: modelIds.slice(0, 30), activity: activityFrom(config, "connection_test", endpoint, startedAt, "failed", ["API credential", "configured model name"], { message }) }, { status: 409 });
    }
    ...
    return respond({ verified: true, modelFound, availableModels: modelIds.slice(0, 30), message, activity: activityFrom(config, "connection_test", endpoint, startedAt, "success", ["API credential", "configured model name"], { message }) });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Provider connection test failed.";
    return respond({ error: message, ...(config && endpoint ? { activity: activityFrom(config, "connection_test", endpoint, startedAt, "failed", ["API credential", "configured model name"], { message }) } : {}) }, { status: 400 });
  }
}
