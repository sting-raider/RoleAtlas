...
export function AccountControls({ user }: { user: { id: string; name: string; email: string } }) {
  ...
  async function removeAccount() {
    ...
    // A full navigation is deliberate: it discards every in-memory workspace
    // state after account deletion, which router.push would keep alive.
  ...
  return (
    <section className="daily-card account-settings-card">
      ...
      <div className="settings-actions">
        ...
        {/* eslint-disable-next-line @next/next/no-location-assign-relative-destination */}
        <button type="button" className="secondary-button" onClick={() => void authClient.signOut({ fetchOptions: { onSuccess: () => { window.location.href = "/sign-in"; } } })}><LogOut size={14} /> Sign out</button>
