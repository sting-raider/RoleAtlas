...
## 2026-08-25, later still — list inputs no longer fight typing (ed4caa5)

The user hit it during real onboarding: "in any of the text boxes while filling details I can't put space or commas n stuff." Root cause: every comma/newline-separated input parsed each keystroke into a string[] and re-rendered from the parsed array — so a trailing space or comma was trimmed away, mid-word commas collapsed words, and "data," became "data" before the next character could land. Affected all 13 onboarding list fields, four strategy-editor fields, and the applications contact field.

Fix inverts ownership: app/listText.ts holds pure split/join/rawMatchesList helpers; app/components/ListInput.tsx keeps the raw keystroke text locally, parses upward only for persistence/live validation, and lets props overwrite the buffer solely on genuine external changes (résumé import, strategy regeneration) using the adjust-state-during-render pattern. ContactInput applies the same contract to the em-dash contact field. Country fields keep name↔code mapping but pass unresolved tokens through (`?? value`) instead of filtering them out, so half-typed country names are never silently dropped mid-word. Five unit tests pin the regression (trailing space/comma survival, case-insensitive dedupe preserving first spelling). Gates: format/typecheck/lint green, 104/104 unit tests, rendered-html contract suite green, rebuilt web container serving with the rewired chunk deployed.
