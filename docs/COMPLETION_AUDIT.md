...
| Notifications/outbox/email abstraction | Partial | Durable owned records with dedupe state; deterministic preference-gated generation for follow-up-due, saved-job closure, and unseen strong-match alerts (migration 0017 preferences; hydration-tick generation proven idempotent and tenant-scoped by the CI-run ignored suite) | Email channel abstraction with local capture, weekly digest, delivery-failure tests |
