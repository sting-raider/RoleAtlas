...
## D-027 — No standing npm-audit exceptions; advisories are closed by bumps, not suppressions

Decision: the dependency tree carries zero known vulnerabilities (`npm audit` clean as of 2026-08-25) and no suppression or waiver mechanism is used. When an advisory appears, the fix lands by bumping the affected package (direct or lockstep transitive) rather than ignoring the advisory. The audit CI job blocks on any finding.

Why: an earlier revision of this decision accepted a temporary red baseline for four high advisories on the assumption next@≥16.3.2 did not yet exist; that premise was wrong — 16.3.2 had shipped inside the existing `^16.2.6` caret range, and the bump (plus a lockstep react/react-dom/react-server-dom-webpack 19.2.8 move for GHSA-wx67-qw84-cm4g) cleared every advisory the same day with no code changes. The episode demonstrated that the honest response to a red audit gate is to attempt the bump first and record a baseline only when no fixed version exists at all.

Rejected: `npm audit` suppressions or waivers in config (invisible decay, hides new findings in the same packages); pinning canary/RC releases to chase fixes; downgrading the audit job to non-blocking. Residual limitation: future advisories may again have no fixed release; any such gap must be recorded here with a dated baseline and an explicit upgrade trigger rather than silently tolerated.

## D-028 — Job discovery is a public read surface; everything else requires a session

Decision: `GET /api/local-scout?action=health|jobs|job` and `GET /api/registry` deliberately require no session — anonymous visitors can search the indexed corpus, open listing details, see source health, and read registry metadata. Every write path (saves, applications, notification acks, workspace, candidate profile, agent runs, résumé uploads) and every personalized or administrative read (stats, metrics, seeds submission) requires a session, with admin role enforced separately.

Why: job discovery is the product's front door — requiring an account to browse listings would contradict the product's purpose, and the corpus contains only public job postings by design. The Phase 8 audit flagged these routes as "unauthenticated reads"; they are intentional, but the intent was undocumented. Recording it makes the boundary reviewable: any NEW route must default to session-required, and moving a route into the public set now needs an explicit decision entry. The Scout-side `/api/jobs` endpoints behind these routes remain unauthenticated for the same reason; they are reachable only on the internal network in production compose.

Rejected: requiring sign-in for search (kills the top-of-funnel; accounts exist to persist state, not to gate browsing); token-gating public reads (adds friction with no security gain — the data is public by construction); leaving the boundary implicit (this audit finding). Residual limitation: rate limiting on public reads is not yet implemented, so the public surface relies on upstream proxy limits at deployment time.

## D-029 — Internal service assertions sign the exact request body
Decision: the HMAC assertion the Next.js web server sends to the Scout api now covers the payload. The canonical signed message is six newline-separated lines — `timestamp`, `METHOD` (uppercased), `path` (with query), `userId`, `role`, and the lowercase hex SHA-256 of the exact request body bytes; an absent or empty body signs the SHA-256 of zero bytes. The Scout side buffers the body of every authenticated route (capped at 4 MiB) inside a `from_fn_with_state` middleware, recomputes the digest, and verifies the HMAC before inserting the authenticated identity; public routes (`/health`, `/api/jobs`, `/api/registry`) bypass the layer entirely.

Why: the Phase 8 audit noted the assertion previously covered method/path/identity/timestamp but not the payload, so any captured signature remained replayable against a different body within the 60-second freshness window — e.g. rewriting a saved-job's `jobRef`. Binding the body closes that window for payloads; the timestamp bound stays 60 s rather than adding nonces, which would require shared state between web replicas and Scout replicas and is judged not worth it while the assertion only ever crosses a single internal Docker network.

Why a middleware: axum 0.8 allows at most one body-consuming extractor per handler and it must be last, so a body-buffering authenticator cannot be expressed as an extractor alongside `Json<T>`; `from_fn_with_state` is the documented pattern, and `AuthenticatedUser` remains a plain `FromRequestParts` extension read.

Cross-language agreement is pinned by tests: a Rust unit test reproduces the exact signature `lib/internal-assertion.ts` emits for fixed inputs, and live probes against the smoke stack showed tampered bodies rejected with 401 while validly signed ones pass (2026-08-25 PROGRESS entry).

Rejected: signing only `content-length` (spoofable, collides across different bodies of equal length); per-request nonces (needs a replay cache or clock sync between services — deferred as unnecessary on one internal network); moving Scout auth to mTLS (bigger operational surface, does not address payload substitution by an in-network actor either).
