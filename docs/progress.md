...
- Known limitations: the verified registry is intentionally 16 boards and is not global market coverage; public feed jobs remain transient and outside crawler reconciliation; live NVIDIA/DeepSeek calls require user credentials and were verified with deterministic mocked provider responses rather than a real key; model proposals cannot automatically add sources; ambiguous eligibility remains `unclear`; and the production web bundle still emits a non-failing size warning above 500 kB.
