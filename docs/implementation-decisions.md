...
50. The legacy stylesheet remains loaded beneath `app/signal-ui.css` during this migration so every working selector keeps its layout/function contract. The new stylesheet owns the visible tokens and component treatment; later deletion of legacy rules must be a separate regression-tested cleanup.
