...
## 2026-07-22 — Work Order 6 (daily usable product)

36. Daily product state is stored as one versioned JSON document per workspace in the additive `daily_workspaces` table. This keeps Work Order 6 backward compatible with the normalized candidate, search-session, provenance, feedback, source-run, and job-history tables rather than duplicating or replacing them.
37. Candidate facts, goals, constraints, preferences, mobility, and search strategy are separate user concepts. Resume-derived evidence is labelled inferred until confirmed; citizenship, authorization, compensation expectations, and relocation willingness are never inferred.
38. Onboarding is a resumable, skippable state machine with reusable adaptive rules. Resume and manual entry are equal paths, every AI action requires an explicit preview/confirmation, and deterministic profile and strategy generation remains available with AI disabled.
39. A persisted active search session is the authoritative Discover scope, including when it legitimately has zero results. Transient public feeds never backfill an empty active session or overwrite its server match, eligibility, provenance, or coverage evidence.
40. Search strategies are user-owned revisioned records. Deterministic regeneration is always available; AI suggestions are previews and cannot silently change a confirmed strategy or hard profile fact.
41. Feedback may adjust soft ranking preferences only through an explained suggestion. Dismissal, undo, all requested reasons, and reset-learned-preferences are persisted; they never rewrite eligibility or candidate facts.
42. Application tracking is organizational assistance, not automation. RoleAtlas supports the complete requested stage taxonomy, next actions, follow-ups, notes, contacts, document references, interview preparation, source status, and activity history, but never submits an application.
43. In-app notifications are durable, deduplicated, dismissible, and read-aware. Work Order 6 deliberately adds no email or push delivery.
44. Service health is product state. The UI and `npm run doctor` distinguish the complete stack from reduced web-only operation and do not present transient public-feed discovery as persistent crawler-backed coverage.
45. Dialogs, drawers, onboarding, and mobile navigation use a shared focus-management contract: initial focus, contained Tab order, Escape dismissal where safe, and focus restoration. Closed mobile navigation is hidden from the accessibility tree, reduced motion is respected, and phone/tablet layouts must not create horizontal overflow.
46. The Work Order 5 source, eligibility, canonical identity, reconciliation, orchestration, and provider-security boundaries remain authoritative. Work Order 6 exposes them through daily workflows; it does not create a second browser-only geography engine or trust model.

## 2026-07-22 — Signal Console UI and public showcase

47. The UI revamp is a presentation-system replacement, not a product-engine rewrite. Existing onboarding, profiles, strategies, search sessions, eligibility decisions, feedback, applications, notifications, sources, and provider flows retain their live handlers and persistence contracts.
48. RoleAtlas adopts a monochrome signal-console language—OLED black, hairline borders, compact Geist/Geist Mono typography, circular dot glyphs, and red only for states requiring attention—without copying proprietary Nothing assets or turning the product into a generic developer dashboard.
49. The 9×9 Opportunity Signal glyph is implemented as semantic-hidden circular HTML cells rather than an image asset. It scales cleanly, respects the product palette, and can reflect running or needs-input states without a bitmap dependency.
50. The legacy stylesheet remained loaded beneath `app/signal-ui.css` during the first migration so every working selector kept its layout/function contract. This transitional decision was superseded by the regression-tested v2 cleanup below.
51. The GitHub Pages site is a dependency-free static product showcase, not a hosted copy of the application. Its sample job and counts are explicitly labelled illustrative because Pages cannot provide PostgreSQL, NATS, crawler, or server-side provider-proxy behavior.
52. Pages deploys from `site/` only when changes reach `master`. Feature branches can validate the artifact without publishing an unfinished redesign, and the application remains separately runnable through the complete local stack.
53. The complete Docker stack sets `ROLEATLAS_SKIP_PUBLIC_FEEDS=true` for its initial server render. Canonical Scout jobs hydrate immediately after the shell loads, while optional public feeds remain available to the standalone development experience; a slow third-party feed can no longer hold the complete product's first HTML response open.

## 2026-07-29 — Nothing UI rebuild v2

54. The legacy visual cascade is retired. Layout-only compatibility rules live in `app/styles/structure.css`; product tokens, reset behavior, and visible component treatment live in `tokens.css`, `base.css`, and `app.css`. This separation keeps the proven responsive layout while preventing old colors, shadows, gradients, typography, and component chrome from leaking back into the product.
55. RoleAtlas supports explicit dark and light themes using one semantic token contract. The theme defaults to the operating-system preference, is stored locally as `roleatlas-theme`, and can be changed from every primary viewport. White/black inversion provides the dominant interaction language; red is reserved for errors, unavailable services, and other states that genuinely require attention.
56. Doto is limited to the RoleAtlas wordmark and signal numerals, Geist and Geist Mono carry the dense product interface, and Newsreader italic is limited to the editorial “Skip the noise” statement. This creates a recognizable visual identity without copying Nothing trademarks or scattering display fonts through operational content.
57. `RoleAtlasApp` is the canonical application-shell name. The historical `FirstRungApp` filename and symbol were removed so current code, tests, and architecture documentation use the shipped product name.
58. The duplicate Scout control modal and placeholder Applications/Profile views were removed because they had no reachable product path. Existing functional Sources, Settings, Applications, and Profile workspaces remain the only supported surfaces.
59. Next-to-Scout API forwarding is centralized in `app/api/scoutProxy.ts`. The refactor preserves the same reduced-mode error behavior and service boundary while eliminating independently drifting URL, response, and failure handling across routes.
60. UI screenshots under `docs/screenshots/ui-v2/` are captures of the real locally running application in reduced mode, not composited mockups. Inventory counts may vary because the standalone development experience uses live supplemental feeds.
